...
const ContextProvider = ({ children }) => {
  ...
  const [downloadPeerTsrBtnStatus, setDownloadPeerTsrBtnStatus] =
    useState(true);
  const [downloadHistoTsrBtnStatus, setDownloadHistoTsrBtnStatus] =
    useState(true);
  ...
  const [historicalTSRTableData2020, setHistoricalTSRTableData2020] =
    useState("");
  const [historicalTSRTableData2019, setHistoricalTSRTableData2019] =
    useState("");
  const [historicalTSRTableData2018, setHistoricalTSRTableData2018] =
    useState("");
  ...
  useEffect(() => {
    const fetchSummaryData = async (id, token) => {
      try {
        const response = await axios.post(
          `${import.meta.env.VITE_APP_BASE_URL}/summary/`,
          { id },
          {
            headers: {
    ...
    if (token) {
      let data = id.map(async (val) => await fetchSummaryData(val, token));
  ...
  useEffect(() => {
    const GoldCorpTableData = async (id, token) => {
      try {
        const response = await axios.post(
          `${import.meta.env.VITE_APP_BASE_URL}/summary_calculated/`,
          { id },
          {
            headers: {
              Authorization: `Bearer ${token}`,
    ...
    if (token) {
      let data = id.map(async (val) => await GoldCorpTableData(val, token));
      Promise.all(data).then((values) => {
  ...
  useEffect(() => {
    const GoldMinerTableData = async (id, token) => {
      try {
        const response = await axios.post(
          `${import.meta.env.VITE_APP_BASE_URL}/cal_bend_value/`,
          { id },
          {
            headers: {
              Authorization: `Bearer ${token}`,
    ...
    if (token) {
      let data = id.map(async (val) => await GoldMinerTableData(val, token));
      Promise.all(data).then((values) => {
  ...
  useEffect(() => {
    const fetchTsrChartData = async (id, token) => {
      try {
        const response = await axios.post(
          `${import.meta.env.VITE_APP_BASE_URL}/histo_tsr/`,
          { id },
          {
            headers: {
              Authorization: `Bearer ${token}`,
    ...
    if (token) {
      let data = id.map(async (val) => await fetchTsrChartData(val, token));
      Promise.all(data).then((values) => {
  ...
  useEffect(() => {
    const fetchPayoutChartData = async (id, token) => {
      try {
        const response = await axios.post(
          `${import.meta.env.VITE_APP_BASE_URL}/payout/`,
          { id },
          {
            headers: {
              Authorization: `Bearer ${token}`,
    ...
    if (token) {
      let data = id.map(async (val) => await fetchPayoutChartData(val, token));
      Promise.all(data).then((values) => {
  ...
  useEffect(() => {
    const TSRBarChartData = async (id, token) => {
      try {
        const response = await axios.post(
          `${import.meta.env.VITE_APP_BASE_URL}/tsr/`,
          { id },
          {
            headers: {
              Authorization: `Bearer ${token}`,
    ...
    if (token) {
      let data = id.map(async (val) => await TSRBarChartData(val, token));
      Promise.all(data).then((values) => {
  ...
  useEffect(() => {
    const fetchHistoricalData = async (id, token) => {
      try {
        const response = await axios.post(
          `${import.meta.env.VITE_APP_BASE_URL}/summary/`,
          { id },
    ...
    if (token) {
      let data = HistoricalDataid.map(
        async (val) => await fetchHistoricalData(val, token)
      );
      Promise.all(data).then((values) => {
  ...
  useEffect(() => {
    const HistoricalTSRTableData = async (id, token) => {
      try {
        const response = await axios.post(
          `${import.meta.env.VITE_APP_BASE_URL}/tsr/`,
          { id },
    ...
    if (token) {
      let data = HistoricalTSRTableDataid.map(
        async (val) => await HistoricalTSRTableData(val, token)
      );
      Promise.all(data).then((values) => {
  ...
  useEffect(() => {
    const downloadTsrFile = async (id, token) => {
      try {
        const response = await axios.post(
          `${import.meta.env.VITE_APP_BASE_URL}/tsr_file/`,
          { id },
          {
            headers: {
              Authorization: `Bearer ${token}`,
    ...
    if (token) {
      let data = id.map(async (val) => await downloadTsrFile(val, token));
      Promise.all(data).then((values) => {
  ...
  useEffect(() => {
    const downloadPeerTsrFile = async (id, token) => {
      try {
        const response = await axios.post(
          `${import.meta.env.VITE_APP_BASE_URL}/peer_tsr_file/`,
          { id },
          {
            headers: {
              Authorization: `Bearer ${token}`,
    ...
    if (token) {
      let data = id.map(async (val) => await downloadPeerTsrFile(val, token));
      Promise.all(data).then((values) => {
  ...
  useEffect(() => {
    const downloadHistoTsrFile = async (id, token) => {
      try {
        const response = await axios.post(
          `${import.meta.env.VITE_APP_BASE_URL}/histo_tsr_file/`,
          { id },
          {
            headers: {
              Authorization: `Bearer ${token}`,
    ...
    if (token) {
      let data = id.map(async (val) => await downloadHistoTsrFile(val, token));
      Promise.all(data).then((values) => {
